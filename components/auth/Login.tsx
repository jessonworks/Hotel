...
import { Hotel, Mail, Lock, AlertCircle, Loader2, RefreshCw, ShieldCheck } from 'lucide-react';

const Login: React.FC = () => {
  const { login, isSupabaseConnected, checkConnection, forceDemoLogin } = useStore();
    ...
    <div className="min-h-screen bg-[#0F172A] flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-8 sm:p-12 rounded-[3rem] shadow-2xl animate-in zoom-in-95 duration-500 relative z-10 border border-white/20">
        <div className="text-center">
          ...
          <p className="text-slate-500 font-bold mt-3 text-sm uppercase tracking-widest">Acesso ao Sistema</p>
        </div>

        {error && (
          <div className="bg-rose-50 border border-rose-100 p-4 rounded-2xl flex items-center gap-3 text-rose-600 text-xs font-black animate-bounce">
        ...
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative">
              ...
              <input 
                ...
                placeholder="gerente@hospedapro.com" 
          ...
          <button 
            type="submit" 
            disabled={loading} 
            className="w-full py-5 bg-slate-900 text-white font-black rounded-3xl shadow-xl hover:bg-blue-700 transition-all transform active:scale-95 flex items-center justify-center gap-3 uppercase tracking-widest text-sm"
          >
        ...
        <div className="pt-4 flex flex-col gap-3">
          <button 
            ...
            className="w-full py-4 border-2 border-dashed border-blue-200 text-blue-600 font-black rounded-3xl flex items-center justify-center gap-2 hover:bg-blue-50 transition-all text-xs uppercase tracking-widest"
          >
            <ShieldCheck size={18} /> Acesso Direto (Gerente)
          </button>
          ...
          >
            ...
            {isSupabaseConnected ? 'Sistema Online' : 'Tentando Reconectar...'}
          </button>
        </div>
      </div>
    </div>
